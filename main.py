...
model = unet()
model_checkpoint = ModelCheckpoint('unet_sarcasm.hdf5', monitor='loss',verbose=1, save_best_only=True)
model.fit_generator(myGene,steps_per_epoch=300,epochs=50,callbacks=[model_checkpoint])

testGene = testGenerator("data/sarcasm/test/image")
results = model.predict_generator(testGene,5999,verbose=1)
saveResult("data/sarcasm/test/image",results)
